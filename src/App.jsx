...
const BUSINESS = {
  name: 'Paradiso Plumbing',
  tagline: "Ballard's straight-talking local plumber",
  phone: '(206) 251-6854',
  email: 'hello@paradisoplumbing.example',
  address: 'Ballard, Seattle, WA',
  hours: 'Mon–Sat, 7am–6pm · Emergency calls anytime',
};

const SERVICES = [
  { icon: Droplet, title: 'Leak Detection & Repair', desc: 'Fast diagnosis and fixes for hidden and visible leaks before they cause real damage.' },
  { icon: Flame, title: 'Water Heater Service', desc: 'Repair, replacement, and tankless installs for reliable hot water year-round.' },
  { icon: Wrench, title: 'Drain & Sewer Cleaning', desc: 'Clogged drains and sewer backups cleared quickly with modern equipment.' },
  { icon: Gauge, title: 'Fixture Installation', desc: 'Faucets, toilets, and sinks installed cleanly, the first time.' },
  { icon: ShieldCheck, title: 'Repipe & Rerouting', desc: "Whole-home repiping for older Ballard homes with aging galvanized pipe." },
  { icon: Clock, title: 'Emergency Plumbing', desc: 'Burst pipe or major leak? We answer the phone and show up.' },
];
    ...
    <>
      ...
      >
        <a href="#top" className="flex items-center gap-2 font-display font-bold text-white">
          <span className="w-8 h-8 rounded-lg bg-accent text-ink flex items-center justify-center font-extrabold">P</span>
          Paradiso Plumbing
        </a>
...
function Hero() {
  ...
  return (
    <header
      id="top"
      className="relative min-h-[100dvh] flex items-center overflow-hidden bg-surface text-white"
    >
      <div
        className="absolute inset-0 bg-cover bg-center"
        style={{
          backgroundImage:
            "url('https://images.unsplash.com/photo-1454988501794-2992f706932e?q=80&w=1920&auto=format&fit=crop')",
        }}
      />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-24">
        <p ref={eyebrowRef} className="eyebrow mb-4">Licensed &amp; Insured · Ballard, Seattle</p>
        <h1 ref={headingRef} className="font-display font-extrabold text-5xl md:text-7xl leading-[1.05] max-w-3xl">
          <span className="block">Plumbing problems,</span>
          <span className="block italic font-flourish font-medium text-accent">solved right</span>
          <span className="block">the first time.</span>
        </h1>
        <p ref={subRef} className="font-body text-white/80 text-lg max-w-xl mt-6">
          Straightforward plumbing repair, installation, and emergency service for Ballard and
          greater Seattle — no upsells, no runaround.
        </p>
        <div ref={ctaRef} className="mt-8">
          <a ref={btnRef} href="#contact" className="magnetic-btn">
            Request Service <Wrench size={18} />
          </a>
        </div>
      </div>
    </header>
              ...
              {Array.from({ length: 7 }).map((_, i) => (
                <Droplet
                  ...
                  style={{
                    left: `${(i * 41) % 100}%`,
                    top: 0,
                    animation: `rain-fall ${2.5 + (i % 4) * 0.6}s linear ${i * 0.4}s infinite`,
                  }}
                />
              ))}
            </div>
            <p className="eyebrow mb-4 relative">Local &amp; Reliable</p>
            <p className="font-body text-sm text-white/60 max-w-[16rem] relative">
              A Ballard-based crew that shows up when they say they will, and stands behind the work.
            </p>
          </div>
          ...
          <div className="rounded-3xl bg-white/5 border border-white/10 p-8 h-72 relative overflow-hidden">
            ...
            <div className="mt-6 grid grid-cols-4 gap-2">
              {Array.from({ length: 16 }).map((_, i) => (
                <div key={i} className={`h-8 rounded-md ${i === 9 ? 'bg-accent' : 'bg-white/10'}`} />
              ))}
            </div>
  ...
  return (
    <section className="bg-paper text-ink py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-6 grid sm:grid-cols-3 gap-10 text-center">
        {[
          { target: 12, suffix: '+', label: 'Years serving Seattle' },
          { target: 100, suffix: '%', label: 'Licensed & insured work' },
          { target: 24, suffix: '/7', label: 'Emergency response' },
        ].map((p) => (
          <div key={p.label}>
...
function Protocol() {
  ...
  const steps = [
    { n: '01', title: 'Call or request online', desc: 'Tell us what\'s going on — we\'ll ask a few questions and give you a realistic window.' },
    { n: '02', title: 'We diagnose on-site', desc: 'A straight answer on what\'s wrong and what it costs before any work starts.' },
    { n: '03', title: 'We fix it right', desc: 'Quality parts, clean work, and a plumber who explains what was done.' },
  ];
...
function TrustSignals() {
  const badges = [
    { icon: ShieldCheck, title: 'Licensed & Insured', desc: 'WA state licensed plumbing contractor.' },
    { icon: Clock, title: '12+ Years Local', desc: 'Serving Ballard and greater Seattle.' },
    { icon: CheckCircle2, title: 'Upfront Pricing', desc: 'You know the cost before we start.' },
  ];
  ...
  return (
    <section id="contact" className="bg-paper text-ink py-24 md:py-32">
      <div className="max-w-3xl mx-auto px-6">
        ...
        <h2 className="font-display text-3xl md:text-5xl font-bold mb-4">Tell us what's going on.</h2>
        ...
        {status === 'sent' ? (
          ...
          <form onSubmit={submit} className="grid sm:grid-cols-2 gap-4">
            ...
            <input required placeholder="Zip Code" className="rounded-xl border border-black/10 px-4 py-3 font-body sm:col-span-1" />
            <textarea required placeholder="What's going on?" rows={4} className="rounded-xl border border-black/10 px-4 py-3 font-body sm:col-span-2" />
            ...
            <button
              ...
              className="magnetic-btn sm:col-span-2 justify-center"
            >
  ...
  return (
    <footer className="bg-surface text-white py-16">
      <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-4 gap-10">
        <div>
          <p className="font-display font-bold text-lg mb-2">Paradiso Plumbing</p>
          <p className="font-body text-sm text-white/60 mb-4">{BUSINESS.tagline}</p>
          <p className="flex items-center gap-2 text-sm text-accent">
            <span className="w-2 h-2 rounded-full bg-accent animate-pulse" /> Currently accepting new jobs
          </p>
        </div>
        <div>
      ...
      <p className="text-center text-white/30 text-xs font-mono mt-12">
        © {new Date().getFullYear()} Paradiso Plumbing. All rights reserved.
      </p>
    </footer>
  );
}
